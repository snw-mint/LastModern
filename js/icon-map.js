(function () {
    ...
    const SHARE_SVG = `<svg class="custom-share-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="18" cy="5" r="3"/><circle cx="6" cy="12" r="3"/><circle cx="18" cy="19" r="3"/><path d="m8.59 13.51 6.83 3.98"/><path d="m15.41 6.51-6.82 3.98"/></svg>`;
    ...
    function injectPrepend(selector, svgContent) {
        document.querySelectorAll(selector).forEach((el) => {
            if (el.querySelector(".custom-reply-icon")) return;
            el.classList.add("has-custom-icon");
    ...
    function injectProfileIcons() {
        ...
        Object.keys(profileMap).forEach((label) => {
            ...
            document.querySelectorAll(selector).forEach((el) => {
                if (el.querySelector(".custom-profile-icon")) return;
                el.classList.add("has-custom-icon");
